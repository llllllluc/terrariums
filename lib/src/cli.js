...
if (argv._[0] === 'deploy') {
    ...
    if (!config.contracts[argv.contract]) {
        error(`Contract ${argv.contract} build information not found in config file.`, { exit: 1 });
    }
    ...
    if (buildInfo.deploy_script) {
        ...
        error(`Contract ${argv.contract} muyst have either a deploy script or instantiate message.`, { exit: 1 });
    }
}
